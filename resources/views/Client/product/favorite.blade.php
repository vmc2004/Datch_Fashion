    ...
    <div class="max-w-screen-xl mx-auto ">
        ...
        <div class="mt-12 md:grid grid-cols-5 gap-8">
            ...
            <div class="col-span-4">
                <div>
                    <h3 class="p-3 bg-white text-slate-800 text-black text-lg md:mb-12 mb-2 border border-solid border-transparent rounded-lg shadow-md shadow-gray-300 hidden md:block"
                        style="width: 920px;">
                                ...
                                <div class="inline-block w-[80px] md:w-48">
                                    <div class="h-full">
                                        <select name="sort"
                                            ...
                                            <option value="-createdAt" {{ $sort == '-createdAt' ? 'selected' : '' }}>Được
                                                tạo gần đây</option>
                                            <option value="createdAt" {{ $sort == 'createdAt' ? 'selected' : '' }}>Cũ nhất
                                                được tạo trước</option>
                                            <option value="updatedAt" {{ $sort == 'updatedAt' ? 'selected' : '' }}>Cập nhật
                                                gần đây</option>
                                            <option value="-updatedAt" {{ $sort == '-updatedAt' ? 'selected' : '' }}>Cũ nhất
                                                cập nhật trước</option>
                                            <option value="totalPrice" {{ $sort == 'totalPrice' ? 'selected' : '' }}>Giá cao
                                                tới thấp</option>
                                            <option value="-totalPrice" {{ $sort == '-totalPrice' ? 'selected' : '' }}>Giá
                                                thấp tới cao</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="flex justify-between p-6">
                                    <div class="flex">
                                        ...
                                        <div class="flex-1 ml-12">
                                            ...
                                            <p class="mb-4 text-black-500">1 Lượt xem, Số lượng:
                                                {{ $favorite->product->productItems->first()->quantity }} (Kích cỡ & Màu
                                                sắc)</p>
                                            <div class="text-black-500">
                                                <span
                                                    class="font-bold mr-12">{{ number_format($favorite->product->productItems->first()->price) }}
                                                    vnđ</span>
                                    ...
                                    <div class="flex flex-col items-center justify-between">
                                        ...
                                        <svg aria-hidden="true" focusable="false" data-prefix="fal"
                                            data-product-id="{{ $favorite->product->id }}" id="trash-icon"
                                            data-icon="trash"
                                            class="w-[20px] h-[20px] overflow-hidden svg-vertical inline-block cursor-pointer"
